...
export default {
  Category:{
    user:async (parent:CategorySchema, __:any, {user: userToken}:Context) =>{
  ...
  Query:{
    categories:async (_:any, {text, page, limit}:{text:string, page: number, limit: number}, {user}:Context): Promise<CategoryPayload> =>{
      ...
      let categories = Category.findAll({ offset: start, limit: +reqLimit, where: { userId: user.id }, order: [['createdAt', 'desc']] })      
      ...
      for(let i= pageCount < 5 ? 1 : fromPage; pageCount < 5 ? i <= pageCount : i<= untilPage; i++){
        allPages.push(i)
      }
    ...
    allCategories: async(_:any, __:any, {user}: Context): Promise<CategoryPayload> =>{
      
      if(!user) throw new ApolloError("User not authorized")
      const categories= await Category.findAll({ where: { userId: user.id }, order: [['createdAt', 'desc']] })
  ...
  Mutation:{
    createCategory: async (_:any, { input }: { input: CategorySchema }, {user}:Context) =>{
    ...
    updateCategory:async (_:any, { id, input }:{ id:string, input: CategorySchema }, {user}: Context) =>{
    ...
    searchCategory: async(_:any, { text, page, limit }: {text: string, page:number, limit:number}, {user}: Context) =>{
