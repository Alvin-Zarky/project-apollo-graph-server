...
sequelize.sync({alter:true})
  .then((url) =>{
    console.log(colors.green.inverse.bold(`Connection Successfully ${process.env.HOST}-${process.env.DATABASE}-${process.env.ROOT}`))
  })
  .catch((err:any) =>{
    console.error(colors.red.underline.bold(`Connection Problem ${err.message}`))
  })
...
server.listen()
  .then(({url}) =>{
    console.log(colors.cyan.inverse.bold(`Server in ${process.env.NODE_ENV} is running on ${url}`))
  })
  .catch((err) =>{
